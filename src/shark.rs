...
impl Shark{
    ...
    pub fn new(texture_path:String, scale: Vector2, left_bound:f32, right_bound:f32) -> Shark{
...
impl Entity for Shark{
    ...
    fn update(&mut self, delta_time: f32){
        ...
        if self.position.x < self.left_bound  { self.move_right(); }
        if self.position.x > self.right_bound  { self.move_left(); }
